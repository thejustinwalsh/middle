...
**Acceptance:** (1) Dispatch an Epic; the agent pauses at a sub-issue (writes blocked.json, exits); the dashboard shows "asked question"; reply on GitHub; the dispatcher signals the workflow; the agent re-spawns with the answer in context and continues from that sub-issue. (2) The agent reaches PR-ready; CodeRabbit (or a human) submits `CHANGES_REQUESTED`; the dispatcher resumes the agent with **all** the pass's review threads in context; it batches them, resolves each (class-wide, tested), runs its internal review loop over the batched diff, then pushes **once**, replies in-thread, and re-requests review; on `APPROVED` the loop ends and the human is pinged to merge; a never-satisfied loop stops at the configured round cap (default 5) in `waiting-human`.

**CI-gated review readiness (red CI is a resume trigger).** A PR is not reviewable until its checks pass, so a **failing `statusCheckRollup` on a ready (non-draft) Epic PR is a resume trigger in its own right** — the same park/poller/resume spine as `CHANGES_REQUESTED`, not a separate mechanism. The poller (item 28) treats failing/errored **required** checks as `changes-requested`-equivalent and resumes the agent with the failing jobs' logs pulled into the brief ("CI is red — investigate and fix these checks"); the agent fixes, pushes **once**, and re-parks. **`review-resolved` additionally requires CI green:** an `APPROVED` / 0-actionable verdict while required checks are red does **not** end the loop (you can't review a PR that doesn't build). Distinguish the states the way the review bump distinguishes paused/reviewing/done: *pending* CI is waited on (it's not a failure), *failing/errored* CI resumes-to-fix, *green* CI is the gate-pass. **Dead/stuck CI** — no run triggered, or a run stuck pending past a timeout — is surfaced to `waiting-human` (a human unsticks it) rather than looped on, since the agent can't fix infrastructure that never ran. Bounded by the same per-repo round cap as the review loop (a perpetually-red PR parks rather than burning rounds forever). This is the CI sibling of the merged/closed-PR reconciliation: both are the poller observing PR/check state and acting on it.
