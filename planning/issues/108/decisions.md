...
## Fix at the source: `INSERT OR IGNORE` over per-step `retry: 1`
**File(s):** `packages/dispatcher/src/workflow-record.ts:49`
**Date:** 2026-05-26

**Decision:** Make `createWorkflowRecord` idempotent with `INSERT OR IGNORE` rather than
adding `retry: 1` to `prepare-worktree`.
...
2. `INSERT OR IGNORE` — the INSERT is the only thing in the step that isn't safe to
