...
- **Review feedback: resolve the class, not the instance.** When a review comment reveals a fragile approach (not a one-off), harden that whole approach within the comment's blast radius (the function/section it touches) in one pass — each adjacent fix with its own test, nothing outside that radius. Fixing only the literal line invites the reviewer to find the next adjacent edge one round at a time (and can blow the auto-loop's round cap). See `implementing-github-issues` → "Addressing review feedback".
